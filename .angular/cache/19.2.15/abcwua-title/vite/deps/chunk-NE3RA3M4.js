...
var DefaultOAuthStore = class {
  ...
  storeOAuthSignIn(oauthSignIn2, preferPrivateSession = false) {
    return __async(this, null, function* () {
      assertTokenProviderConfig(this.cognitoConfig);
      const authKeys = createKeysForAuthStorage2(name, this.cognitoConfig.userPoolClientId);
      yield this.keyValueStorage.setItem(authKeys.oauthSignIn, `${oauthSignIn2},${preferPrivateSession}`);
    });
  }
};
...
function handleSelectChallengeWithPasswordSRP(username, password, clientMetadata, config, session, tokenOrchestrator2) {
  return __async(this, null, function* () {
    ...
    if (response.ChallengeName === "PASSWORD_VERIFIER") {
      return retryOnResourceNotFoundException(handlePasswordVerifierChallenge, [password, response.ChallengeParameters, clientMetadata, response.Session, authenticationHelper, config, tokenOrchestrator2], activeUsername, tokenOrchestrator2);
    }
    return response;
  });
}
...
var updateUserAttributes = (input) => __async(null, null, function* () {
  const {
    userAttributes,
    options
  } = input;
...
var updateUserAttribute = (input) => __async(null, null, function* () {
  const {
    userAttribute: {
      attributeKey,
      value
...
var handleImplicitFlow = (_0) => __async(null, [_0], function* ({
  ...
  const {
    ...
    error_description,
    error
  } = (url.hash ?? "#").substring(1).split("&").map((pairings) => pairings.split("=")).reduce((accum, [k, v]) => __spreadProps(__spreadValues({}, accum), {
    [k]: v
  }), {
    id_token: void 0,
    access_token: void 0,
...
var oauthSignIn = (_0) => __async(null, [_0], function* ({
  oauthConfig,
  provider,
  clientId,
  customState,
  ...
  const queryString = Object.entries(__spreadValues(__spreadProps(__spreadValues(__spreadValues(__spreadValues({
    redirect_uri: redirectUri,
...
function signOut(input) {
  return __async(this, null, function* () {
    ...
    if (hasOAuthConfig) {
      ...
      const {
        type
      } = (yield handleOAuthSignOut(cognitoConfig, oAuthStore2, tokenOrchestrator, input?.oauth?.redirectUrl)) ?? {};
      if (type === "error") {
        throw new AuthError({
...
var parseDevicesResponse = (devices) => __async(null, null, function* () {
  return devices.map(({
    DeviceKey: id = "",
    DeviceAttributes = [],
    DeviceCreateDate,
...
//# sourceMappingURL=chunk-NE3RA3M4.js.map
